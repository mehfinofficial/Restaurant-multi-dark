...
let pendingOutlet = null;

function openConfirmSheet(outlet) {
    pendingOutlet = outlet;

    document.getElementById("confirmSheet").style.display = "block";
...
function loadRestaurantInfo(){
    document.getElementById("outletDisplay").innerText =
    `${selectedOutlet.name}`;
...
subtotal += unitPrice * cartItem.qty;
    ...
    let delivery = deliveryMode === "delivery" ? selectedOutlet.deliveryFee : 0;
    ...
    let summaryHTML = `
        ...
        ${deliveryMode === "delivery" 
            ? `<div class="rowLine"><span>Delivery</span><span>₹${selectedOutlet.deliveryFee}</span></div>` 
            : ""
        }
...
document.querySelectorAll(".mode-btn").forEach(btn => {
    btn.addEventListener("click", () => {
        ...
        if (deliveryMode === "delivery") {
            deliveryFee = selectedOutlet.deliveryFee;
...
`;
    ...
    let businessNumber = selectedOutlet.phone;  
...
function openOutletSheet() {
    document.getElementById("outletSheet").style.display = "block";

    setTimeout(() => {
        document.getElementById("outletSheet").classList.add("show");
    }, 10);

    renderOutletList();
}

function closeOutletSheet() {
    document.getElementById("outletSheet").classList.remove("show");

    setTimeout(() => {
        document.getElementById("outletSheet").style.display = "none";
    }, 300);
}
...
function renderOutletList() {
    ...
    menuData.outlets.forEach(o => {
        html += `
        <div class="outlet-item ${selectedOutlet.id === o.id ? 'active' : ''}"
             onclick="selectOutletFromSheet('${o.id}')">

            <div class="outlet-name">${o.name}</div>
            <div class="outlet-meta">${o.location} • ${o.timing}</div>
        </div>
        `;
    });
...
function selectOutletFromSheet(id) {
    const outlet = menuData.outlets.find(o => o.id === id);
